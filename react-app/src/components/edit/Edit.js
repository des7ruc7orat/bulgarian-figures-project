...
export const Edit = () => {
  ...
  //  const [figure, setFigure] = useState({});
    ...
    const submitHandler = async(e) => {
        ...
        try {
            ...
            const {firstName,secondName,familyName,yearBorn,yearDied,nickname,imageUrl,
                  occupation, description} = values;
                  if(values.secondName == ''){
                    values.secondName = 'none'
                }
                  const data =  {firstName,secondName,familyName,yearBorn,yearDied,nickname,imageUrl,
                      occupation, description};
                   
                    //  console.log(data);
                const res = await figureService.updateFigureById(figureId, data)
      ...
      navigate(`/details/${figureId}`)
       
    //  const resFigure =  await figureService.updateFigureById(figure._id,figureData)
    ...
    return (
        <>
            <h1 className={styles.homeTitle}>Edit a person</h1>
            ...
            <div className={styles.registerBox} >
                ...
                <form onSubmit={submitHandler}>
                    ...
                    <label className={styles.labelCreate}
                        htmlFor="secondName">Second Name:
                    </label>
                    <input
                    ...
                    <label
                        ...
                        htmlFor="familyName">Family Name:
                    </label>
                    <input
                        className={styles.input} type="text" name="familyName"
                        id="familyName" value={values.familyName}
                        onChange={changeHandler}
                        onBlur={(e) => minLength(e, 5)}
                        placeholder="Popovich"
                    />
                    {errors.familyName &&
                        <p className={styles.error}>
                    ...
                    <label className={styles.labelCreate}
                        htmlFor="yearBorn">Year born:</label>
                    <input className={styles.input} type="number" name="yearBorn"
                        id="yearBorn" value={values.yearBorn} onChange={changeHandler}
                        onBlur={handleInputChange} placeholder="1821"
                    />

                    <label
                        className={styles.labelCreate}
                        htmlFor="yearDied">Year of death:
                    </label>
                    <input
                        className={styles.input} type="number"
                        name="yearDied" id="yearDied" value={values.yearDied}
                        onChange={changeHandler} onBlur={handleInputChange}
                        placeholder="1867"
                    />
                    ...
                    <input className={styles.input} type="text"
                        ...
                        placeholder="Georgi Sava Rakovski"
                    />
                    {errors.nickname &&
                        <p className={styles.error}>
